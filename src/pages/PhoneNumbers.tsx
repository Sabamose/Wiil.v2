...
const PhoneNumbers = () => {
  ...
  const handleUnassign = (numberId: string) => {
    ...
    setPhoneNumbers(phoneNumbers.map(num => num.id === numberId ? {
      ...num,
  ...
  const handleAssignComplete = (phoneNumberId: string, assistantId: string, assistantName: string) => {
    ...
    setPhoneNumbers(phoneNumbers.map(num => num.id === phoneNumberId ? {
      ...num,
      assignedAssistant: {
        id: assistantId,
        name: assistantName
  ...
  return <div>
        ...
        <div className="p-4 md:p-8">
          ...
          <div className="bg-white rounded-lg border overflow-hidden">
            <div className="overflow-x-auto">
              ...
              <TableBody>
                {phoneNumbers.map(phoneNumber => <TableRow key={phoneNumber.id} className="cursor-pointer hover:bg-gray-50 transition-colors duration-200" onClick={() => handlePhoneNumberClick(phoneNumber)}>
                    ...
                    <TableCell>
                      {phoneNumber.assignedAssistant ? <span className="text-gray-900 text-sm">{phoneNumber.assignedAssistant.name}</span> : <span className="text-teal-600 hover:text-teal-700 transition-colors duration-200 text-sm font-medium cursor-pointer underline decoration-teal-300 decoration-dashed underline-offset-2 hover:decoration-solid">
                          {isMobile ? "Connect" : "Click here to Connect Assistant"}
                        </span>}
                    </TableCell>
                    <TableCell className="hidden sm:table-cell">
                      <span className={`inline-flex px-2 py-1 text-xs font-medium rounded-full ${phoneNumber.status === "active" ? "bg-green-100 text-green-800" : "bg-gray-100 text-gray-800"}`}>
                    ...
                    <TableCell>
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end" className="bg-white border shadow-lg z-50">
                          {phoneNumber.assignedAssistant && <DropdownMenuItem onClick={e => {
                          e.stopPropagation();
      ...
      <PhoneNumberPurchaseModal isOpen={isPurchaseModalOpen} onClose={() => setIsPurchaseModalOpen(false)} onPurchaseComplete={handlePurchaseComplete} />

      <AssignAssistantModal isOpen={isAssignModalOpen} onClose={() => setIsAssignModalOpen(false)} phoneNumber={selectedPhoneNumber} onAssignComplete={handleAssignComplete} />
    </div>;
};
